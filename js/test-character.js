...
const waitForLoad = setInterval(() => { //setting a interval to make the characterTesting() wait for async fetches to complete
    if (!itemsObject || !biggerItemsObject || !classesObject || !backgroundsObject || !originFeatsObject || !speciesObject/*&& !otherObject || !anotherObject */){ //our check to see if all the async variables our character depends on have actually loaded
        tries++;
